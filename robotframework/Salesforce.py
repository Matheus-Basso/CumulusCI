...
class Salesforce(object):
    ...
    def product_adder(self, uid, quote_name, product_list):
        url = 'SBQQ/ServiceRouter?loader=SBQQ.QuoteAPI.QuoteProductAdder'
        product_list = product_list.replace('"configured":false', '"configured":true')
        product_list = product_list.replace('"configurationEntered":false', '"configurationEntered":true')
        product_list = product_list.replace('"', '\\"')
        product_list = product_list.replace("\\r\\n", '')
        # print(product_list)
        body = '{"context":"{\\"quote\\":{\\"record\\":{\\"attributes\\":{\\"type\\":\\"SBQQ__Quote__c\\",\\"url\\":\\"/services/data/v41.0/sobjects/SBQQ__Quote__c/' + uid + '\\"},\\"Name\\":\\"' + quote_name + '\\",\\"Id\\":\\"' + uid + '\\"},\\"nextKey\\":2,\\"netTotal\\":0.00,\\"lineItems\\":[],\\"lineItemGroups\\":[],\\"customerTotal\\":0.00},\\"products\\":[' + product_list + '],\\"groupKey\\":0, \\"ignoreCalculate\\": true}"}'
        body = body.replace("\\r\\n", '')
        body = body.encode('ascii', 'ignore').decode()
